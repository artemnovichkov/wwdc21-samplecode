...
class PreviewView: UIView, UIGestureRecognizerDelegate {
	...
	private func commonInit() {
		...
		/*
         ...
         view so that the region of interest can resize and move. If you would
         like to have a fixed region of interest do not add the following gesture
         recognizer. You need to set the region of interest once.
		*/
	...
	/*
	 ...
	 touch threshold to avoid control selection conflicts when a user tries
	...
	*/
	...
	/*
	 ...
	 the KVO notification triggers when the resizing finishes.
	*/
	...
	/*
	 ...
	 a new region of interest is set, the region of interest redraws.
	*/
	func setRegionOfInterestWithProposedRegionOfInterest(_ proposedRegionOfInterest: CGRect) {
		// Standardize to ensure positive widths and heights with an origin at the top left.
		...
		/*
		 Intersect the video preview view with the view's frame to get only
		...
		case .changed:
			...
			case .none:
				...
				/*
				 ...
				 translate only the region of interest in the plane that
	...
	override func layoutSubviews() {
		...
		// Create the path for the mask layer. Use the even-odd fill rule so
        // that the region of interest doesn't have a fill color.
	...
	func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        // if and only if the touch location is not within the touch threshold.
		if gestureRecognizer == resizeRegionOfInterestGestureRecognizer {
